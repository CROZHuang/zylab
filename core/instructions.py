...
今天已经在生效的东西——旧规则是"往上找到第一个 CLAUDE.md 就停"，在
`/root/data/zylab` 里干活时命中的正是 `/root/data/CLAUDE.md`。停在 git 根
等于把这台机器的规矩（只读挂载、网络路由）从 zylab 眼前拿走，而用户没要求过
这件事。所以继续爬，用体量提示（`WARN_TOTAL_CHARS`）让人自己决定要不要减。
"""
